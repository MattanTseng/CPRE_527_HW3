...
class Parallel_Net_500k(nn.Module):
    def __init__(self):
        super(Parallel_Net_500k, self).__init__()
        # Convolutional layers
        self.conv1 = nn.Conv2d(3, 32, 3, padding=1).to('cuda:0')
        self.conv2 = nn.Conv2d(32, 64, 3, padding=1).to('cuda:0')
        # Max-pooling layers
        self.pool = nn.MaxPool2d(2, 2).to('cuda:0')
        # Fully connected layers
        self.fc1 = nn.Linear(64*8*8, 512).to('cuda:1')
        self.fc2 = nn.Linear(512, 256).to('cuda:1')
        self.fc3 = nn.Linear(256, 10).to('cuda:1')
